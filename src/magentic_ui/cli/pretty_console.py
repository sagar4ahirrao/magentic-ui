...
# │  Helper utilities                                                          │
# ╰────────────────────────────────────────────────────────────────────────────╯
DEBUG_PRINT = False  # Set to True to enable additional debug prints, hardcoded on purpose
...
def format_plan(obj: dict[str, Any], colour: str) -> None:
    ...
    if (
        ...
        if isinstance(step_complete, dict) and "answer" in step_complete:
            status = f"{GREEN}Yes{RESET}" if step_complete["answer"] else f"{YELLOW}No{RESET}"
    ...
    # --- need to find where to print the progress ledger
        ...
        if "step_type" in obj:
            step_type = (
                "Sentinel Step" if obj["step_type"] == "SentinelPlanStep" else "Regular Step"
            )
            print(left)
...
def pretty_print_plan(raw: str, colour: str) -> bool:
    ...
    if not ok:
        return False
    if any(k in obj for k in ("task", "plan_summary", "steps", "title", "is_current_step_complete", "agent_name")):
...
def format_web_surfer_actions(raw: str, colour: str) -> bool:
    ...
    if not isinstance(raw, str) or "The actions the websurfer performed are the following" not in raw:
        return False
    ...
    async def process(msg: BaseChatMessage | BaseAgentEvent | TaskResult | Response):
        ...
        try:
            ...
            if isinstance(msg, BaseChatMessage):
                ...
                if src.lower() == "web_surfer" and format_web_surfer_actions(content, colour):
                    pass
